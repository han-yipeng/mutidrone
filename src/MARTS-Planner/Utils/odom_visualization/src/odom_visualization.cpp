...
static double traj_scale_x, traj_scale_y, traj_scale_z, traj_color_r, traj_color_g, traj_color_b, traj_color_a;
...
ros::Publisher meshShowPub;
visualization_msgs::MarkerArray meshShow_array;
...
void odom_callback(const nav_msgs::Odometry::ConstPtr& msg)
{
  ...
  if ((msg->header.stamp - prevt).toSec() > 0.01)
  {
  ...
  trajROS.scale.x = traj_scale_x;
  trajROS.scale.y = traj_scale_y;
  trajROS.scale.z = traj_scale_z;
  trajROS.color.r = traj_color_r;
  trajROS.color.g = traj_color_g;
  trajROS.color.b = traj_color_b;
  trajROS.color.a = traj_color_a;  
  ...
  if (_isQuadrotor)
  {
    ...
    meshROS.scale.x = 0.75 * scale * 1.45;
    meshROS.scale.y = 0.75 * scale * 1.45;
    meshROS.scale.z = 0.75 * scale * 1.45;
  ...
  else
  {
    ...
    q(0) = msg->pose.pose.orientation.w;
    meshROS.pose.orientation.w = q(0);
    meshROS.scale.x = 0.25 * scale * 1.3;
    meshROS.scale.y = 0.25 * scale * 1.3;
    meshROS.scale.z = 0.25 * scale * 1.3;
    meshROS.color.a = color_a;
    meshROS.color.r = color_r;
    meshROS.color.g = color_g;
    meshROS.color.b = color_b;
...
int main(int argc, char** argv)
{
  ...
  n.param("traj/scale/x", traj_scale_x, 0.1);
  n.param("traj/scale/y", traj_scale_y, 0.0);
  n.param("traj/scale/z", traj_scale_z, 0.0);

  n.param("traj/color/r", traj_color_r, 0.0);
  n.param("traj/color/g", traj_color_g, 1.0);
  n.param("traj/color/b", traj_color_b, 0.0);
  n.param("traj/color/a", traj_color_a, 0.6);
