...
public:
	...
	Odom_Data_t laserodom_data;
	Odom_Data_t feetodom_data;
	Imu_Data_t imu_data;
	Command_Data_t cmd_data;
	Command_transport_Data_t cmd_transport_data;
	Battery_Data_t bat_data;
	Takeoff_Land_Data_t takeoff_land_data;
	RPM_Data_t rpm_data;
	ExtForce_Data_t neural_force_data;
	ExtForce_Data_t indi_force_data;

	Controller &controller;

	ros::Publisher traj_start_trigger_pub;
	ros::Publisher ctrl_FCU_pub;
	ros::Publisher ctrl_FCU_pub_actuator;
	ros::Publisher debug_pub; //debug
	ros::Publisher gedebug_pub; //debug
	ros::ServiceClient set_FCU_mode_srv;
	ros::ServiceClient arming_client_srv;
	ros::ServiceClient reboot_FCU_srv;

	quadrotor_msgs::Px4ctrlDebug debug_msg; //debug
	quadrotor_msgs::geDebug gedebug_msg;
...
private:
	...
	Desired_State_t get_cmd_des(const int &task_mode);
	...
	void publish_attitude_ctrl_with_bodyrate_feedforward(const Controller_Output_t &u, const ros::Time &stamp);
	void publish_bodyrate_ctrl(const Controller_Output_t &u, const ros::Time &stamp);
	void publish_actuator_ctrl(const Controller_Output_t &u, const ros::Time &stamp);
