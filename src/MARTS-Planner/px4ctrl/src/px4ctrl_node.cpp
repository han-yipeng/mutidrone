...
int main(int argc, char *argv[])
{
    ...
    // // cout << "abcdf\t" << param.task_mode  << endl;
    ...
    //                                                   ros::VoidConstPtr(),
    //                                                   ros::TransportHints().tcpNoDelay());                       // for eight
    // }
    // else if (param.task_mode == 1)
    // {
    ros::Subscriber cmd_transport_sub =
        nh.subscribe<quadrotor_msgs::PositionCommandNew>("cmd",
                                                         100,
                                                         boost::bind(&Command_transport_Data_t::feed, &fsm.cmd_transport_data, _1),
                                                         ros::VoidConstPtr(),
                                                         ros::TransportHints().tcpNoDelay());         // for transport 
    // }
    ...
    fsm.ctrl_FCU_pub_actuator = nh.advertise<mavros_msgs::ActuatorControl>("/mavros/actuator_control", 10);
    
    fsm.traj_start_trigger_pub = nh.advertise<geometry_msgs::PoseStamped>("/traj_start_trigger", 10);

    fsm.debug_pub = nh.advertise<quadrotor_msgs::Px4ctrlDebug>("/debugPx4ctrl", 10); // debug
    fsm.gedebug_pub = nh.advertise<quadrotor_msgs::geDebug>("/gedebugPx4ctrl", 10); // debug
