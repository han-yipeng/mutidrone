...
void PX4CtrlFSM::process()
{
	...
	case AUTO_HOVER:
	{
		// cout << "rc_data.is_command_mode:" << rc_data.is_command_mode << endl;
		// cout << "cmd_is_received(now_time):" << cmd_is_received(now_time) << endl;
		...
		else if (rc_data.is_command_mode && cmd_is_received(now_time))
		{
			...
			{
				state = CMD_CTRL;
				des = get_cmd_des(param.task_mode);
	...
	else
	{
		...
		case 3:    // indi controller for single quadrotor without drag
			debug_msg = controller.update_alg3(des, odom_data, rpm_data, imu_data, u, rc_data.is_hover_mode, rc_data.is_command_mode);
		    debug_msg.header.stamp = now_time;
		    debug_pub.publish(debug_msg);
			break;
		case 4:  // indi controller for transportation 
			debug_msg = controller.update_alg4(des, odom_data, rpm_data, imu_data, u, rc_data.is_hover_mode, rc_data.is_command_mode, bat_data.volt);
			debug_msg.header.stamp = now_time;
			debug_pub.publish(debug_msg);
			// printf("pub alg4 debug\r\n");
			break;

		// case 5:  // indi controller for transportation 
		// 	debug_msg = controller.update_alg5(rc_data.thrust_ch, rpm_data, imu_data, u);
		// 	debug_msg.header.stamp = now_time;
		// 	debug_pub.publish(debug_msg);
		// 	// printf("pub alg4 debug\r\n");
		// 	// cout << "2222222222" << endl;
	...
	switch (param.ctrl_mode)
	{
	case 0:
		// u.thrust = 0.0;
		publish_attitude_ctrl(u, now_time);
		// printf("not publish_attitude_ctrl\r\n");
		break;

	case 1:
		publish_attitude_ctrl_with_bodyrate_feedforward(u, now_time);
		break;
	...
	case 3:
		publish_actuator_ctrl(u, now_time);
		break;
	...
	// ltime = ros::Time::now();
	// cout << (ltime - now_time).toSec() << endl;
	// if ((ltime - now_time).toSec() > 0.004)
	// {
	// 	ROS_ERROR("!!!!!!!dura: %.7lf",(ltime - now_time).toSec());
	// }

	// static ros::Time endtime = ros::Time::now();
	...
	// else
	// {
	// 	double dura = (ros::Time::now() - endtime).toSec();
	// 	cout << dura << endl;
	// 	if(dura > 0.005)
	// 		ROS_ERROR("!!!!!!!!!!!%.7lf",dura);
	// }
...
Desired_State_t PX4CtrlFSM::get_cmd_des(const int &task_mode)
{
	Desired_State_t des;
	if (task_mode == 0)    // for eight  
	{
		des.p = cmd_data.p;
		des.v = cmd_data.v;
		des.a = cmd_data.a;
		...
		des.yaw_acc = cmd_data.yaw_acc;
	}
	else if (task_mode == 1)   // for auto takeoff
	{
		if ((int)cmd_transport_data.flag == 0)   // without Fq feedforward
		{
			// cout << "22222222222" <<endl;
			des.p = cmd_transport_data.pl;
			des.v = cmd_transport_data.vl;
			des.a = cmd_transport_data.al;
			des.j = cmd_transport_data.jl;
			des.s = cmd_transport_data.sl;
			des.yaw = cmd_data.yaw;
			des.yaw_rate = cmd_data.yaw_rate;
			des.yaw_acc = cmd_data.yaw_acc;
			des.flag = 0;
		}
		if ((int)cmd_transport_data.flag == 1)   // current indi Fq act as feedforward
		{
			des.p = cmd_transport_data.pl;
			des.v = cmd_transport_data.vl;
			des.a = cmd_transport_data.al;
			des.j = cmd_transport_data.jl;
			des.s = cmd_transport_data.sl;
			des.q = Eigen::Vector3d::Zero();
			des.dq = Eigen::Vector3d::Zero();
			des.d2q = Eigen::Vector3d::Zero();
			des.d3q = Eigen::Vector3d::Zero();
			des.d4q = Eigen::Vector3d::Zero();
			des.F = 0.0;
			des.dF = 0.0;
			des.d2F = 0.0;
			des.yaw = cmd_data.yaw;
		...
		else if ((int)cmd_transport_data.flag == 2)   // with Fq feedforward
		{
			des.angT = cmd_transport_data.angT;
			des.angT(1) += 2 * (param.drone_id - 1) * M_PI / param.drone_num;
			des.cst = getcs(des.angT(0));
			des.csp = getcs(des.angT(1));
			des.dAngT = cmd_transport_data.dAngT;
			des.d2AngT = cmd_transport_data.d2AngT;
			des.d3AngT = cmd_transport_data.d3AngT;
			des.d4AngT = cmd_transport_data.d4AngT;
			des.q = getQ(des.cst, des.csp, des.gs);
			des.dq = getdQ(des.dAngT.head(2), des.cst, des.gs);
			des.d2q = getd2Q(des.dAngT.head(2), des.d2AngT.head(2), des.cst, des.gs);
			des.d3q = getd3Q(des.dAngT.head(2), des.d2AngT.head(2), des.d3AngT.head(2), des.cst, des.gs);
			des.d4q = getd4Q(des.dAngT.head(2), des.d2AngT.head(2), des.d3AngT.head(2), des.d4AngT.head(2), des.cst, des.csp, des.gs);
			des.F = param.scale * des.angT(2);
			des.dF = param.scale * des.dAngT(2);
			des.d2F = param.scale * des.d2AngT(2);

			des.p = cmd_transport_data.pl + param.l * des.q;
			des.v = cmd_transport_data.vl + param.l * des.dq;
			des.a = cmd_transport_data.al + param.l * des.d2q;
			des.j = cmd_transport_data.jl + param.l * des.d3q;
			des.s = cmd_transport_data.sl + param.l * des.d4q;
			des.yaw = cmd_data.yaw;
			des.yaw_rate = cmd_data.yaw_rate;
			des.yaw_acc = cmd_data.yaw_acc;
			des.flag = 2;
...
void PX4CtrlFSM::set_hov_with_rc()
{
	...
	double max_height_vel = 0.2;
	double max_yaw_vel = 0.2;
	...
	hover_pose(2) += rc_data.ch[2] * max_height_vel * delta_t * (param.rc_reverse.throttle ? 1 : -1);
	hover_pose(3) += rc_data.ch[3] * max_yaw_vel * delta_t * (param.rc_reverse.yaw ? 1 : -1);
...
// {
// 	// cout << (now_time - cmd_data.rcv_stamp).toSec() << endl;
...
{
	// cout << (now_time - cmd_data.rcv_stamp).toSec() << endl;
	return (now_time - cmd_transport_data.rcv_stamp).toSec() < param.msg_timeout.cmd;
}
...
void PX4CtrlFSM::publish_attitude_ctrl_with_bodyrate_feedforward(const Controller_Output_t &u, const ros::Time &stamp)
{
	mavros_msgs::AttitudeTarget msg;
	...
	Eigen::Vector3d bodyrate = u.bodyrates;
	msg.body_rate.x = bodyrate.x();
	msg.body_rate.y = bodyrate.y();
	msg.body_rate.z = bodyrate.z();
...
void PX4CtrlFSM::publish_actuator_ctrl(const Controller_Output_t &u, const ros::Time &stamp)
{
	...
	for(int i = 0; i <= 3; i++)
	{
		msg.controls[i] = u.motor_thrust[i];
	}
	...
	ctrl_FCU_pub_actuator.publish(msg);
}
